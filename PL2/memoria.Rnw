...
Para el segundo ejercicio, realizaremos un análisis de asociación con Arules para cun conjunto de datos que leeros a través de un archivo CSV.
...
Importamos de nuevo el paquete \emph{arules} para el análisis Apriori.
...
Cargamos los datos del csv en una variable que llamaremos “datos”. Para ello utilizamos la función "scan".
<<>>=
...
Observamos la matriz generada a partir de los datos del csv.
...
Mostramos las asociaciones de productos que cumplen los umbrales de soporte y de confianza establecidos.

\section{Conclusiones}

Hemos realizado un análisis de asociación de datos con R, usando el algoritmo Apriori proporcionado por el paquete \emph{arules}. Para el ejercicio 2 hemos propuesto un análisis de asociación para un conjunto de datos que leemos mediante un archivo CSV para los cuáles calcularemos las asociaciones con soporte 60\% y confianza 80\%.
